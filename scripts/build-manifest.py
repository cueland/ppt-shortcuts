#!/usr/bin/env python3
"""
Generate manifest.xml: shared runtime, Home-tab "Open" button, and the Expropriated Elements
...
VERSION = "0.1.0.0"
SHORTCUTS_V = "8"   # bump when shortcuts.json changes
ICON_V = "3"        # bump when any icon PNG changes (Office caches by URL)
...
RIBBON = [
    ...
    ("Tools", "similar", [
        ...
        B("togglePane", "Pane", "pane", "Show / hide the Expropriated Elements pane"),
    ]),
]
...
short["S.blank"] = "\u00a0"
sid("tab", "Expropriated")
sid("home.group", "Expropriated Elements"); sid("home.open", "Open"); lid("home.open.desc", "Open the Expropriated Elements pane: icons, key assignment, settings, log.")
sid("gs.title", "Expropriated Elements loaded"); lid("gs.desc", "Shortcuts are registered. Press Ctrl+Shift+Option+K or use the Expropriated Elements tab.")
...
<OfficeApp xmlns="http://schemas.microsoft.com/office/appforoffice/1.1"
  ...
  <DisplayName DefaultValue="Expropriated Elements"/>
  <Description DefaultValue="Expropriated Elements: keyboard-driven shape tools for PowerPoint on macOS."/>
  <IconUrl DefaultValue="{BASE}/assets/icon-32.png?v={ICON_V}"/>
  <HighResolutionIconUrl DefaultValue="{BASE}/assets/icon-80.png?v={ICON_V}"/>
...
{tab_xml}
    ...
    <Resources>
      <bt:Images>
        <bt:Image id="Icon.16x16" DefaultValue="{BASE}/assets/icon-16.png?v={ICON_V}"/>
        <bt:Image id="Icon.32x32" DefaultValue="{BASE}/assets/icon-32.png?v={ICON_V}"/>
        <bt:Image id="Icon.80x80" DefaultValue="{BASE}/assets/icon-80.png?v={ICON_V}"/>
{image_res()}
      </bt:Images>
      <bt:Urls>
